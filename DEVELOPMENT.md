...
4.  **Screens (`lib/<feature>/screen.dart`)**: Independent presentation layer extending `ScreenState<T>`. 
    * **Screen Rules & Blob Separation of Concerns**:
      * **Read & Listen Only**: Screens read and listen to `Blob` instances (e.g., via `ListenableBuilder` or `ValueListenableBuilder`) to automatically re-render reactive UI components when data changes.
      * **No Direct Blob Mutations in Views**: Screens **must never** mutate state or invoke `Blob.update(...)` directly from UI handlers. All state-changing operations (adds, edits, toggles, deletes, setting updates) must be defined as public methods on the concrete `Module` class (accessed via `widget.module.<method>()`), which in turn mutates the underlying `Blob`. This enforces a clean single-direction command flow and keeps business logic, validation, and side-effects encapsulated within the module layer.
    * Screens **never** interact directly with the Bluetooth connection, execute platform method channels, or write directly to the database. They delegate updates to the `widget.module` reference.
